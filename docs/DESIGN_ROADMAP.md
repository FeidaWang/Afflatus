...
第三批（移动端，`src/styles.css` 末尾 `@media(max-width:860px)` 最终块）：

- **Logo**：`.brand-mark` 直接用 `/favicon.svg` 作背景（去掉旧的 clip-path 三角 + 细节伪元素），缩到 26px，与 favicon 完全统一；wordmark 缩到 14px；移动端隐藏 version 小字。
- **顶栏精简**：隐藏 `.top-telemetry` 遥测条（移动端纯噪声），收紧 padding/gap，nav-links 居中压缩，腾出浏览空间。
- **HUD 精简**：`--hud-combat-h` 降到 `min(40svh,300px)`，面板 padding 收紧、label 缩小；terminal（hud-right）与 battle cam（hud-pilot）功能完整保留。

第四批（修正与回退）：

- **回退 landing cockpit**：`drawCockpitFrame` 遮挡视野且 HUD 难看，已移除 landing/combat 两处调用，恢复旧版 pilot HMD。（函数体保留为未用代码，可删。）尾焰增强保留。
- **Defense-module battle log 修复**：之前 ~6px、只静态显示最新一条 → 读起来像"两个点没播报"。改为只显示 `.active`（JS 每 3.2s 轮播）形成实时滚动，字号提到 9px 可读。
- **CIWS 准心抖动**：`drawCiwsCamera` 的瞄准点改为 lerp 平滑，准心滑向目标而非逐帧抖。
- **Strip 计数器抖动**：count-up 动画每帧改变数字宽度、非等宽字体会换行 → 整条 strip 抖动。`.strip-value` 锁 `tabular-nums` + `nowrap` + `min-height`，布局不再 reflow。

第五批（主炮星舰，`src/scene/capitalFlyby.js`）：

- **重做舰体造型**（对照 fig2）：细长星驱匕首 → 厚重工业舰体（平直甲板 + 密集 greeble + 双甲板沟槽）。决定继续用 2D 离屏图层（移动端零 GPU 开销）；WebGL 留给将来全屏大尺寸过场。
- **尾部四引擎舱**：圆柱舱 + 喷口钟罩，`drawEngineGlow` 改为白热核心 + 长尾等离子焰，推进器清晰可见。共享 `ENGINES`/`STERN_X` 常量供烘焙与实时辉光复用。
- **舰首主炮**：带肋环的重型炮管 + 炮口膛环，充能时膛口红色聚能。

第六批（CIWS，`src/main.js`）：

- **Phalanx 近防炮**：新增 `drawPhalanxMount`——前景白色雷达罩 + 弹鼓箱体 + 6 管加特林炮管簇,瞄准彗星方向,开火时炮管旋转 + 炮口爆闪,曳光弹从真实炮口射出。替换了旧的抽象 gimbal 圆环与画面底角曳光。准星 lerp 平滑保留。

第七批（威胁标记 + 主炮镜头，`src/main.js` / `src/scene/capitalFlyby.js`）：

- **威胁读数**：`updateCursorTarget` 按彗星速度方向把读数框放到「前导侧」（尾迹拖在速度反方向），并在屏幕边缘自动翻面，避免被华丽彗尾遮挡；保留单实例去重。
- **主炮充能放慢**：充能 3.0s → 4.5s、开火窗口 1.7s → 2.4s、cam 时长 → 9.2s。
- **镜头编排**：cam 改为读真实 `getCannonFx()` 时间线——充能阶段播放 `capitalFlyby` 慢速侧掠，并把 flyby 方向翻成**左→右**(先尾部推进器、再扫过舰首主炮)，开火瞬间切到炮口 cam。
- **光束**：`drawMainGunCamera` 光束方位角改为对齐彗星在主页上的方位（`atan2(curY-中心, curX-中心)`），光束**穿过目标贯穿整屏**、加粗（外辉 w*.26 / 核心 w*.15），开火瞬间快速展开。

第八批（battle log 真修 + 主炮光束 + Terminal）：

- **Battle log 根因**：`.toast.active` 用了 `animation:logEnter .4s`（缺 `forwards`），动画结束后回退到基样式 `opacity:0` → 文字消失只剩瞬间的点。改为 `opacity:1 / animation:none`，并给 `.msg` 显式颜色。这才是"两个点没文字"的真因。
- **主炮光束**：改为从画面下方中心（= 主页母舰中心 `innerWidth*.5,innerHeight+420` 的对应原点）射出，穿过目标贯穿整屏，方位角与主页一致。
- **Terminal 星图**：`starMapScene` 用面板真实宽高比投影（`Rx=w*.48,Ry=h*.47`），不再是居中小圆；右上角标签从 `T-countdown` 改为实时地球距离（AU，与左下角"days"不重复）；login 按钮 CSS 已在右上角。

待办：

- Terminal hover 双指针（无法盲修，需现场复现指针 DOM）。
- 主炮星舰造型进一步贴合 fig1（Sulaco 重舰）——画布美术，需现场逐帧调。
