...
用途：MSI 期间每日一跑（赛后触发），更新 `leagues-data.json`——已完场系列赛复盘计分 + 剩余场次预测 + 夺冠概率。7/12 决赛后收官一跑转 `mode:"archived"`。
...
纪律：胜负概率成对归一；夺冠概率全场归一；赔率 = 1/p 的诚实换算并标注 "implied"（非真实盘口）；每完场一个系列赛立即复盘上期预测（命中/比分/Brier）。输出单个 JSON；仅供娱乐、非博彩建议。
