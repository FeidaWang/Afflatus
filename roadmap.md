...
0. **N1**《玉熙宫词》第一部 500 章落地执行（2026-07-07 立项，**当前最高优先级**）——**v7 脚本已定稿（2026-07-13）**于 `prompts/yuxi-150-script.md`：站主裁决第一部扩为 **500 章（十节×5幕×10章）、总量维持 ~30 万字、均章 ~600 字微章体裁、从头全量重写**（已发 120 章降级为底稿）；分幕总表与锚点章号见脚本 §4.0，批次=一幕 10 章顺写、整节替换发布（书签作废为已接受代价）。v6 资产（幕宇宙论/史料弹药库/心理档案/历练技）全部继承。**批1（节一幕一，新 #1–10，5773字）已写，草稿存 `prompts/yuxi-v7-drafts/jie01.json`**，待节一满 5 幕后整节替换 `public/novels/yuxi-gongci.json`。执行方式：说「写节一幕二」继续。
