...
1. **V22** 移动端全局修复（站主 2026-07-06 拿 iPhone 16 Pro Max Chrome 实机截图立项）——**六项代码全部完成 ✅（2026-07-06），待真人浏览器验收**，详见 `RELEASE_NOTES.md`；完整规格与验收表见 **§7.9**。
...
6. **V21** 观星台「治愈系」大改版 + 运势娱乐功能矩阵（参照测测 App 立项，2026-07-06）——**Phase 0–6 全部完成 ✅（2026-07-06，详见 `RELEASE_NOTES.md`，视觉整体待真人浏览器验收）**。仅存后续决策项：行星星座（引星历库 vs 手写系数的取舍，见 §7.8.3 表注）、紫微辅星/四化，完整规格与分期见 **§7.8**。**注意 C5 约束：任何 Phase 若要开新页，必须先做 Astro 评估。**
7. **V2** Games 世界杯收官跟进——被动监控，决赛 7/19，赛程推进后补 `home/away/result` 即可。
8. **机会主义拾取**（无截止压力，不强制排期）：C1/C2 Signal 传导链可视化自动化（§6，暂不可行，卡点见 §6）> ~~B7 Games 夺冠之路树状图~~ / ~~B9 Odin 贴花~~ ✅ 均已完成，详见 `RELEASE_NOTES.md` > B3 combatHudSC 纵深透视（记录在案，不建议单独立项）。
...
### 7.9 V22 移动端全局修复（2026-07-06 立项，站主实机截图驱动）

> **证据基础**：站主提供 iPhone 16 Pro Max Chrome 三张实机截图（index / serial / horoscope）。这是全站第一次真机移动端反馈——此前所有验收都在桌面视口做，暴露的问题都是真实的用户可见破损。**验证约束（诚实声明）**：沙盒无法渲染页面，修复只能靠 CSS 推理 + 窄视口静态检查，**每一项修完都需要站主在真机上复核**，截图即验收标准。

#### 7.9.0 逐项问题与修复方案

**① 全站：移动端两侧翻页箭头删除（S）**
顶部导航已经存在，两侧固定箭头（`public/page-turn.css` 的 `.page-turn-controls`）在窄屏上挤占内容还遮挡文字（三张截图全部可见）。方案：`@media (max-width:760px)` 直接 `display:none`（文件里已有一个 760px 断点可挂）；桌面端不动。翻页能力由顶部 nav 完全覆盖，无交互损失。

**② 首页 hero 字号 + 底部 HUD 死区（M，谨慎级）**
- `index.html:215` 的 `.hero-title` 是硬 `<br>` 三行 + 大字号，390px 视口下单词级溢出换行（截图里 "I command the fleet" 撑爆）。方案：字号改 `clamp()` 下限收紧；硬 `<br>` 在窄屏 `br{display:none}` 让自然换行接管。
- 底部 HUD 卡片（雷达/星图）下方存在整屏高度的空黑区、无显示无交互——疑似 combat/HUD canvas 在移动端保留了桌面布局高度但内容不渲染。**这是 main.js/styles.css 深水区（V15/B6 纪律：WebGL/canvas 改动沙盒验证不了）**，方案分两档：保守档（首选）只动 CSS——移动端把该区块高度收掉/隐藏；激进档（改 JS 渲染分支）不在本次范围。
- 顶部 COMMANDER MODE 提示条在移动端占位过大，顺手收窄。

**③ 顶部栏移动端菜单按钮（M）**
站主要求：移动端顶部栏加一个按钮，点开可通往各子页。现状 `src/lib/nav.js` 的顶层导航在移动端可能收纳不全（首页顶栏只见 CRUISE 和时钟）。方案：`nav.js` 增加 `≡` 汉堡按钮（≤760px 显示），点开全屏/下拉面板列出全部 8 页（复用现有 `SITE` 数组与 Labs 分组数据，portal 到 body 的既有模式），各页自动继承——一处改动全站生效。注意每页配色身份：面板用中性深底+各页 accent 沿用 Labs 下拉已有的 CSS 变量机制。

**④ 小说页：进度条与工具栏之间的缝隙（S，桌面端同修）**
根因已定位：`public/styles/serial.css` 里 `.reader-progress{position:sticky;top:0;height:3px}` 而 `.toolbar{position:sticky;top:8px}`（移动端 `top:4px`）——中间 5-8px 缝隙滚动时漏出正文，沉浸感断裂，桌面移动都有。方案：工具栏 `top:3px` 与进度条无缝衔接（或进度条并入工具栏顶边），两端一起修。

**⑤ 小说页：移动端工具栏两行 → 一行（M）**
现状工具栏 flex-wrap 换行成两行（截图：目录/三个色圆/字号一行，自动翻页/慢中快/瀑布流/书签第二行）。方案：≤640px 改紧凑图标化单行——目录 `≡`、配色三圆点保留（本来就小）、字号 `A±` 合并为一个循环按钮或弹出小面板、自动翻页 `▶` 点击后才展开慢/中/快、瀑布流用图标、书签 `☆`；溢出项收进「⋯」更多菜单。原则：常用操作一击可达，低频操作允许二级。

**⑥ 观星页：移动端表单重排（M，"灾难"级反馈）**
截图问题清单：时区 select 冲出奶油色面板背景（intrinsic 宽度：option 文案太长 + `.fld select{min-width:120px}` 不许收缩）；出生日期与对方生日期宽度不一致；字段/按钮/复选框各自为政（夏令时 checkbox 悬在纬度输入旁边，起盘按钮嵌在经度行里），视觉完全不统一。方案（`public/styles/horoscope.css`，≤640px）：
- `.form-grid` 改单列 `grid`，**所有输入/选择器统一 `width:100%`、同高（44px 触控友好）、同圆角**；`select` 加 `max-width:100%` + `text-overflow:ellipsis`，时区 option 文案缩短（"UTC+8 · 中国"级别即可，长名录留桌面）。
- 字段顺序重排成逻辑组：出生日期+时辰 → 性别 → 时区+夏令时（同组同行）→ 经纬度（同组同行，两个半宽）→ 起盘按钮独占一行全宽。
- 合盘表单同规则；两个表单宽度自然一致。
- 桌面端布局不动（改动全部包在断点内）。
