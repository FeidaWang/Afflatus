...
0. **N1**《玉熙宫词》第一部 30 章落地执行（2026-07-07 立项，**当前最高优先级**）——**v8 脚本已定稿（2026-07-14）**于 `prompts/yuxi-150-script.md`：站主裁决改为 **30 章 × 均章 ~1.33 万字 ≈ 40 万字**（十节 × 3 章），无限流强情节定调（智斗 boss 局/多反转/配角高光/团队合作/打怪升级），游戏化系统上线（副本规则书/香火积分结算/道具卡/灵魂契约同契），穿越机制改十次往返（每节入幕出幕+现实间隔写实，衔接自检清单）；《大明王朝1566》全二册 PDF 已全文提取入库（楔子+40章逐章定位，脚本 §7.6 原著实读清单）。**男女主更名（2026-07-14）**：陆时砚→白道微（拼音声母 B-D-W）、阮棠→常灵珂（拼音声母 C-L-K），女主母亲设定改为洁癖+家教严+唠叨（原设定为病榻缠绵）；感情线改为六阶段起承转合版（接触了解→亲密战友→互相依赖→灵魂伴侣→暧昧情愫→克制没点破，§5.3），每阶段甜虐各一场。第 1–3 章已扩写到位（12407/11212/10578字）并同步完成改名，第 4 章起沿用「已发 120 章」旧底稿素材库（旧名未改，仅供取材参考）。执行方式：说「写第 N 章」，一章一批整章替换。
...
| Sectors | `sectors.html` | AI + 航天个股研判 ＋ v1.5 中美 AI 对比矩阵、「后内存时代」专题（已上线，数据待攒，§7.2） | **2026-07-18 起改版**：openai.com 现行浅色极简风格（白底黑字+青色强调+大图 hero+方形新闻卡网格+mega-footer），站主指令确认为本页独有、与全站暗色 HUD 身份不一致的例外（详见 `Urgent.md` U35） | 顶层 |
...
| Novels | `serial.html` | 小说连载书架：《万界种春》（无限流·种田）＋《长夜请柬》（无限流·悬疑推理）＋《玉熙宫词》（无限流·明穿权谋，第一部30章×40万字脚本v8·重写中（旧120章为底稿）），书架切换 | 复古未来主义 · 铜/青（纯中文，护眼阅读，含夜间/豆沙绿/米黄三种阅读模式、自动翻页、书签、瀑布流、章节速览） | **Labs** |
