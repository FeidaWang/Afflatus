...
>
> **NEXT UP · 建议执行队列（2026-07-04 四次重排——D1-D5 插入最前）**——标注工作量（S≈半天内 / M≈1–3 天 / L≈1 周级）：
> ⓪ **D1 → D2 → D3 → D4 → D5**（各 S，**当前最高优先级，正在做**）——见下方 P0★ 分组，逐条说明。
...
- **D1.（S）`/api/quote` 加白名单 + per-IP 限流**——现状：`api/quote.js` 只用 regex 校验 symbol 格式（`^[A-Za-z.\-]{1,12}$`），任意字符串都能打上游 Finnhub；`Cache-Control: s-maxage=12` 只对相同 symbol 有效，不同 symbol 各自吃一次配额。免费档限流是**全站访客共享**的，开盘时段两个并发访客就可能互相打出 429。**方案**：symbol 白名单复用 `public/arena-universe.json` 的 30 支（域外直接 400，不打上游）；加一个基于 IP（`req.headers['x-forwarded-for']`）的简单滑动窗口限流（Vercel serverless 无持久内存，可先用一个粗粒度的内存 Map + 短 TTL，配额吃紧再评估 Upstash/KV）。`api/history.js` 同款漏洞，一并加白名单。
...
- **D3.（S）首页背景渲染修复——吸收 B5**——现状：`vite build` 每次都报 `[INEFFECTIVE_DYNAMIC_IMPORT] src/scene/topdownCombat.js is dynamically imported by src/main.js but also statically imported by index.html`，代码分割实际失效，一直没人处理；首页背景 canvas 不可见时（切标签页/滚出视口）仍在渲染，移动端耗电。**方案**：把 `topdownCombat.js` 的引用方式统一成一种（要么 main.js 也改静态导入，要么 index.html 改成动态加载点），构建日志里这条警告消失即验证通过；背景 canvas 加 `IntersectionObserver` + `document.visibilityState` 双重挂起（原 P3 **B5** 并入本条，不再单独跟踪）。
- **D4.（S）移动端触控目标 + 最小字号一轮 pass**——现状：nav 链接实测约 30px 高、`.tf-b` 36px、V13 新增的 `.ta-chip` 更紧凑，均低于 44px 触控指南；HUD 标签大量 9-10px 字号低于可读底线。**方案**：全站 `.nav a`/`.hbtn`/`.tf-b`/`.ind-b`/`.ta-chip`/`.ta-mode` 等交互元素 min-height/padding 提到 44px 等效点击区（视觉大小可保持不变，用 padding/伪元素扩热区）；9px 系标签在 `@media(max-width:720px)` 下提到 ≥11px。逐页过一遍，不是只改 Arena。
- **D5.（S）sitemap.xml + i18n 首帧防闪**——现状：`public/robots.txt` 存在但无 `sitemap.xml`；中文访客每次进页先看到英文原文再被 `i18n.js` 客户端替换成中文，且爬虫只抓到英文（中文内容对 SEO 不存在）。**方案**：生成静态 `sitemap.xml`（7 个入口页）并在 `robots.txt` 里指向它；i18n 首帧闪烁的根治需要服务端按 `Accept-Language` 或路径分流（这类是构建/部署层面的改动），本条先做**能立刻做的部分**——`<html lang>` 初始值按 localStorage 缓存的语言提前设置、关键首屏文案避免整段先英后中的跳变，SSR/预渲染级别的根治留给 C5 Astro 迁移一并解决。
...
- **B8. V14 相机导演剩余 polish**——`impactOrbit`（末段撞击环绕镜头）未实现；FOV 动态推拉/banking 侧倾未实现；「空间深度四件套」（实例化尾焰彩带/运动模糊粒子/近景尘埃视差层/动态 FOV）完全未动。这些是纯视觉细节，且需要真人在浏览器里确认观感才知道值不值得做，v1 切片（`?combatcam=director`）验收后再排期。详见 `RELEASE_NOTES.md` V14 条目。
