...
> ③ **V6**（M）→ ④ **V7**（S）——**插入到 V4 之后、V5 之前**：这两项不依赖 V4 的账本数据，正好填满 V4 启动后到 V5 解锁前的等待窗口，比严格串行 V4→V5→V6→V7 更快出活，且仍能赶在 7/28-29 FOMC 前上线（原有 ~3–4 天工作量，窗口期够用）。
> ⑤ **V5**（M，账本满 3 个交易日后再动工）→ 进入 P2。
>
> 想接着做时直接说编号（如「做 V4」）。
...
- **V4.（M）Arena 双模型定时任务**——Model A 日内双窗口批处理（开盘后 + 尾盘各 1 次）+ Model B 盘后 1 次 + 周六深度复盘 1 次/周；所有任务先过交易日历守卫（周末/NYSE 假日 no-op）；提示词 `prompts/arena-autopilot.md`（已就绪）。调度、数据获取与 key 管理见 **§7.5**（2026-07-04 增补：用 launchd 替代 crontab——睡眠错过的任务唤醒后补跑，cron 直接跳过）。
- **V6.（M）Signal「Warsh 时代」内容重构**——新主席 SCP 人事档案 + 五维信号矩阵（通胀/货币政策/财报指引/产业动向/地缘贸易）+ 无前瞻指引 ⇒ 数据发布日历权重提升 + 鹰鸽罗盘首版。规格见 **§7.3**。**软截止：7 月 28–29 日 FOMC 前上线**（已核实，见上）。提示词 `prompts/signal-warsh.md`（已就绪）。**现状**：`signal.html` 的「NEXT CONTAINMENT TEST」卡片已手动更新到"Warsh 第二次会议"这条真实事件（说明倒计时机制本身在正常运作），但五维信号矩阵/鹰鸽罗盘首版等结构性内容尚未做，V6 要做的是这部分。
