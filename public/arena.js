/* ============================================================
   ...
   Pre-open direction game on US AI-complex equities. You vs. an
   explainable model. Live Finnhub quotes, simulator fill for any
   symbol the feed can't supply, a daily pre-market briefing, and a
   trading-desk style historical chart.
...
(() => {
  ...
  const CONFIG = {
    ...
    pollMs: 12000,           // 11 symbols / 12s ≈ 55 calls/min (under the 60/min free cap)
    newsUrl: '/arena-news.json',
  };

  // The AI-complex roster. SPCX + FLKR are house tickers (no public feed) and
  // are driven by the simulator; the rest pull live quotes when available.
  const TICKERS = [
    { symbol: 'NVDA', name: 'NVIDIA',       sector: 'AI Compute',  seed: 142.0 },
    { symbol: 'SPCX', name: 'SpaceX',       sector: 'Space',       seed: 180.0 },
    { symbol: 'MU',   name: 'Micron',       sector: 'AI Memory',   seed: 132.0 },
    { symbol: 'SNDK', name: 'SanDisk',      sector: 'Storage',     seed: 48.0 },
    { symbol: 'FLKR', name: 'Flicker AI',   sector: 'AI Software', seed: 64.0 },
  ...
  const HOUSE = new Set(['SPCX', 'FLKR']); // always simulated
  ...
  function scoreText(t) { const a = tok(t); if (!a.length) return 0; let s = 0; for (const w of a) { if (POS.includes(w)) s++; if (NEG.includes(w)) s--; } return Math.max(-1, Math.min(1, s / 4)); }
  ...
  // ---- seeded historical series (so charts read pre-open) -----
  function strHash(s) { let h = 2166136261; for (let i = 0; i < s.length; i++) { h ^= s.charCodeAt(i); h = Math.imul(h, 16777619); } return h >>> 0; }
  function mulberry32(a) { return function () { a |= 0; a = a + 0x6D2B79F5 | 0; let t = Math.imul(a ^ a >>> 15, 1 | a); t = t + Math.imul(t ^ t >>> 7, 61 | t) ^ t; return ((t ^ t >>> 14) >>> 0) / 4294967296; }; }
  function seedHistory(t) {
    const rnd = mulberry32(strHash(t.symbol));
    let price = t.seed * (0.9 + rnd() * 0.05);
    const drift = (rnd() - 0.42) * 0.004, out = [], now = Date.now();
    for (let i = 0; i < SEED_LEN; i++) {
      const shock = (rnd() - 0.5) * 0.02;
      price = Math.max(0.5, price * (1 + drift + shock));
      out.push({ t: now - (SEED_LEN - i) * 60000, price: Number(price.toFixed(2)), seed: true });
    }
    return out;
  }
  ...
  function finnhubProvider(key) {
    ...
    async function one(sym) {
      ...
      return { symbol: sym, price: q.c, prevClose: q.pc || q.c, open: q.o || q.c, high: q.h || q.c, low: q.l || q.c, live: true };
    }
    return { name: 'finnhub', async fetchQuotes(syms) { const res = await Promise.allSettled(syms.map(one)); const out = {}; res.forEach((x, i) => { if (x.status === 'fulfilled') out[syms[i]] = x.value; }); return out; } };
  }
  function simProvider(tickers) {
    ...
    function step(s) { const j = Math.random() < 0.015 ? g() * s.vol * 6 : 0; s.price = Math.max(0.5, s.price * (1 + s.drift + sent * 0.0006 + g() * s.vol + j)); s.high = Math.max(s.high, s.price); s.low = Math.min(s.low, s.price); return { price: +s.price.toFixed(2), prevClose: +s.prevClose.toFixed(2), open: +s.open.toFixed(2), high: +s.high.toFixed(2), low: +s.low.toFixed(2), live: false }; }
    return { name: 'sim', setSentiment(v) { sent = v; }, async fetchQuotes(syms) { const out = {}; for (const s of syms) if (st[s]) out[s] = { symbol: s, ...step(st[s]) }; return out; } };
  }

  // ---- state --------------------------------------------------
  const history = Object.fromEntries(TICKERS.map((t) => [t.symbol, seedHistory(t)]));
  const latest = {};
  ...
  function loadGame() { try { const r = localStorage.getItem(STORAGE_KEY); const s = r ? JSON.parse(r) : null; return s ? { player: { ...emptySide(), ...s.player }, ai: { ...emptySide(), ...s.ai }, rounds: s.rounds || [] } : { player: emptySide(), ai: emptySide(), rounds: [] }; } catch { return { player: emptySide(), ai: emptySide(), rounds: [] }; } }
  const g0 = loadGame();
  const state = { selected: SYMBOLS[0], sentiment: 0, news: { date: null, items: [], loading: true }, lastUpdate: 0, liveCount: 0, player: g0.player, ai: g0.ai, rounds: g0.rounds, round: null, result: null, remaining: 0 };
  ...
  // ---- feed (live + simulator fill) ---------------------------
  let liveProvider = (CONFIG.dataSource === 'finnhub' && CONFIG.finnhubKey) ? finnhubProvider(CONFIG.finnhubKey) : null;
  ...
  async function poll() {
    ...
    try {
      let live = {};
      const liveSyms = SYMBOLS.filter((s) => !HOUSE.has(s));
      if (liveProvider) { try { live = await liveProvider.fetchQuotes(liveSyms); } catch {} }
      const simq = await sim.fetchQuotes(SYMBOLS);
      let liveCount = 0;
      for (const s of SYMBOLS) {
        const q = (!HOUSE.has(s) && live[s]) || simq[s]; if (!q) continue;
        if (q.live) liveCount++;
        const prev = latest[s];
  ...
  function resolveRound({ direction, startPrice, endPrice, confidence = 0.6, isAI = false }) {
    ...
    const points = correct ? 10 + (isAI ? Math.round(confidence * 10) : Math.round(Math.min(10, movePct * 4))) : -4;
  ...
  function renderWatchlist() {
    ...
    host.innerHTML = TICKERS.map((t) => {
      ...
      const tag = HOUSE.has(t.symbol) ? '<span class="tk-house">SIM</span>' : (q?.live ? '<span class="tk-live">LIVE</span>' : '');
  ...
  function renderChart() {
    ...
    $('cBadge').textContent = st.state === 'open' ? 'LIVE' : 'PRE-MARKET TAPE';
    $('cBadge').className = 'chart-badge ' + (st.state === 'open' ? 'is-live' : '');
    ...
    const v = hist.map((h) => h.price), pc = q ? q.prevClose : v[v.length - 1];
    const lo = Math.min(...v, pc), hi = Math.max(...v, pc), pad = (hi - lo) * 0.12 || 1, LO = lo - pad, HI = hi + pad, RG = HI - LO || 1;
    ...
    const pcy = Y(pc); grid += `<line x1="0" y1="${pcy.toFixed(1)}" x2="${plotW}" y2="${pcy.toFixed(1)}" class="cpc"/><text x="2" y="${(pcy - 4).toFixed(1)}" class="cpc-t">PREV ${pc.toFixed(2)}</text>`;
  ...
  function renderPred() {
    ...
    if (r) {
      ...
      host.innerHTML = `<div class="live-wrap">
        ...
        <div class="pxs"><span>Entry <b>$${r.startPrice.toFixed(2)}</b></span><span>Now <b class="${live >= r.startPrice ? 'up' : 'down'}">$${live.toFixed(2)}</b></span></div></div>`;
    } else if (res) {
      host.innerHTML = `<div class="live-wrap">
        ...
        <div class="again"><span class="lbl">Go again</span><button class="bet up" data-dir="UP">▲ UP</button><button class="bet down" data-dir="DOWN">▼ DOWN</button></div></div>`;
  ...
  function renderScore() {
    ...
    const host = $('rounds');
    host.innerHTML = state.rounds.length ? state.rounds.map((r) => `<div class="rd"><span style="font-weight:700">${r.symbol}</span><span class="${r.actual === 'UP' ? 'up' : 'down'}">${r.actual === 'UP' ? '▲' : '▼'}</span><span class="ch ${r.player.correct ? 'ok' : 'bad'}">You ${r.player.correct ? '✓' : '✗'}</span><span class="ch ${r.ai.correct ? 'ok' : 'bad'}">AI ${r.ai.correct ? '✓' : '✗'}</span></div>`).join('') : '<div class="empty">No rounds yet — make your first call.</div>';
  }
  ...
  function buildBriefing() {
    const n = state.news, sl = sentLabel(state.sentiment);
    const icon = { financial: '◇', industrial: '▣', political: '⬡', tech: '⊞' };
    const items = n.items.map((it) => {
      const s = sentLabel(typeof it.sentiment === 'number' ? it.sentiment : scoreText(`${it.title} ${it.summary || ''}`));
      return `<article class="bf-item"><div class="bf-itop"><span class="bf-cat">${icon[it.category] || '•'} ${it.category || 'note'}</span><span class="bf-sent ${s.tone}">${s.label}</span></div><h3 class="bf-title">${it.title || ''}</h3>${it.summary ? `<p class="bf-sum">${it.summary}</p>` : ''}${it.source ? `<a class="bf-src" href="${it.url || '#'}" target="_blank" rel="noreferrer noopener">${it.source} ↗</a>` : ''}</article>`;
    }).join('');
    return `
    <div class="bf-backdrop" id="bfBackdrop"></div>
    <div class="bf-modal" role="dialog" aria-modal="true" aria-labelledby="bfHeading" tabindex="-1" id="bfModal">
      <div class="bf-prog" id="bfProg" role="progressbar" aria-label="Reading progress" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0" tabindex="0"><div class="bf-prog-fill" id="bfProgFill"></div></div>
      <header class="bf-head">
        <div class="bf-barcode" aria-hidden="true"></div>
        <div class="bf-htext">
          <p class="bf-kicker">TRAXUS//CVKM · DESK BRIEFING</p>
          <h2 class="bf-h" id="bfHeading">Pre-Market Briefing</h2>
          <p class="bf-date">${n.date ? 'Digest · ' + n.date : 'Digest · awaiting first run'} · sentiment <b class="${sl.tone}">${sl.label}</b></p>
        </div>
        <button class="bf-x" id="bfClose" aria-label="Skip briefing and close">✕</button>
      </header>
      <div class="bf-body" id="bfBody" tabindex="0">
        <div class="bf-warn" role="note"><b>⚠ NOT INVESTMENT ADVICE.</b> This briefing and the Arena are for entertainment and education only. Nothing here is a recommendation to buy or sell any security. Prices may be delayed or simulated; house tickers (SPCX, FLKR) are fictional.</div>
        <p class="bf-intro">Today's signal across financial, tech, political and industrial wires for the US AI complex. Read it, form a view, then call each name's direction before the bell.</p>
        ${items || '<div class="bf-empty">No digest yet. The scheduled task writes <code>public/arena-news.json</code> about an hour before the US open each weekday.</div>'}
        <div class="bf-end">— END OF BRIEFING —</div>
      </div>
      <footer class="bf-foot">
        <button class="bf-skip" id="bfSkip">Skip</button>
        <button class="bf-enter" id="bfEnter" disabled>SCROLL TO READ ▾</button>
      </footer>
    </div>`;
  }
  function openBriefing() {
    ...
    const body = $('bfBody'), fill = $('bfProgFill'), prog = $('bfProg'), enter = $('bfEnter');
    let read = false;
    const update = () => {
      const max = body.scrollHeight - body.clientHeight;
      const pct = max > 0 ? Math.min(100, (body.scrollTop / max) * 100) : 100;
      fill.style.width = pct + '%'; prog.setAttribute('aria-valuenow', Math.round(pct));
      if (pct >= 96 && !read) { read = true; enter.disabled = false; enter.textContent = 'ENTER THE ARENA ▶'; }
    };
    body.addEventListener('scroll', update);
    // draggable / clickable progress (also keyboard via arrows)
    const scrub = (clientX) => { const r = prog.getBoundingClientRect(); const ratio = Math.max(0, Math.min(1, (clientX - r.left) / r.width)); body.scrollTop = ratio * (body.scrollHeight - body.clientHeight); };
    let dragging = false;
    ...
    $('bfEnter').addEventListener('click', close);
    $('bfClose').addEventListener('click', close);
    $('bfSkip').addEventListener('click', close);
    $('bfBackdrop').addEventListener('click', close);
    host.addEventListener('keydown', (e) => { if (e.key === 'Escape') close(); }, { once: false });
  ...
  fetch(CONFIG.newsUrl, { cache: 'no-store' })
    ...
    .then((data) => {
      ...
      sim.setSentiment(state.sentiment);
    })
