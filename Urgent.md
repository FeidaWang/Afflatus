...
- [x] ㉑ 移动端审计 ✅ 代码级审计已完成（2026-07-18，真机截图部分仍待站主，见下）：逐页核对 9 页（sectors/signal/games/league/stats/arena/horoscope/serial/index）的多列网格 + 横向元素的断点覆盖。**发现并修复 1 处真·破版**：`public/styles/sectors.css` 的 `.macro`（首页宏观指标 5 列条）在 820px 只降到 2 列，此后一路停在 2 列到手机宽度——补了 `@media(max-width:480px){.macro{grid-template-columns:1fr}}` 单列降级。**核查后记录 1 处仅拥挤、非破版**（不动）：`horoscope.css` 的 `.zw-grid`（紫微命盘 4 列）轨道是 `1fr` 无最小宽度，窄屏只是挤但不溢出，不算"破版"，不在本项施工范围内。其余 8 页抽查均已有 ≤768px（或更早）断点覆盖多列网格，未发现新破版。**「横屏查看」提示组件：核查后未落地，原因记录**——spec 原文点名的两处（games 淘汰赛树、stats bootstrap 分布图）经代码核查**均已有自己的降级手段**：games.html 的 `.ko-tree`（4+ 阶段树状图）已有 `overflow-x:auto` 横向滚动；stats.html 的 `#bootHist` 是 `viewBox` + `width:100%` 的响应式 SVG，本来就随容器缩放，不是固定像素宽度。两处都不存在"横屏才能看清"的真实场景，此时再加一个提示组件是无落点的空转——按 Simplicity First 不为假想用途预先造轮子；若后续真出现放不下的可视化（比如 U43 的区间刷子未来加宽），再按需接回这个组件设计。
- [x] ㉒ serial.html 阅读模式 ✅ 已完成（2026-07-18）。**范围核查修正**：spec 原文按"暗（现状）/纸白/护眼米黄三档"立项当作新功能，但 serial.html 的阅读器**早就有三档主题**（`.reader[data-theme]`：`imperial` 默认深色暖调、`green` 浅绿纸感、`night` 深色琥珀夜读——U15e 就已经为夜读特意压过白点亮度），工具条已有切换钮（`toolbar` 内按钮，见 serial.html:100 起）、`localStorage` 持久化、`transition` 过渡全部齐活，不是本项要新建的东西。核查内容改为：① **AA 对比度校验**（WCAG 相对亮度公式实测）——三档主文字对比度 8.53–10.77:1、次要文字（`--read-ink-dim`）4.65–6.77:1，全部超过 AA 4.5:1 门槛（多数还过了 AAA 的 7:1）；② **排版规格核对**——`line-height` 已是 1.95（早超 1.7 下限），但**行长没有上限**，`.wrap` 900px 减去内边距后正文实际跑到约 90+ 字符/行，超过 U41 在 signal.html 立的 68ch 规矩。补了一行 `public/styles/serial.css`：`.reader .body{max-width:68ch}`，`.pad`/`.chapter-nav`/标题不受影响（本来就该比正文列宽）。717 vitest 全绿，build 无新增报错。
...
- [x] ⑮ 特性①：stardrive exit 段收缩 + 字幕交棒（与既有 pin keyframes 合并核对，真机重点看 pin→shrink 交界帧）。✅ 代码已完成（2026-07-18），**对规格文字的两处必要修正，如实记录**：
  - **落点改回 legacy 层，不进 `@layer components`**：现场读代码才发现 `.stardrive-stage` 的 pin 动画（`stardrivePin`）本身就活在 `@layer legacy` 里（U21 迁移前的老代码，整块 `.stardrive` 样式都还没搬），而且新的 `stageShrink` 和它是**同一条 `animation` 简写声明里的第二个动画**（`animation:stardrivePin linear both,stageShrink linear both`）——没法把一条声明拆到两个层里。层顺序是 `legacy, tokens, components, overrides`，components 在 legacy 之后，若把 shrink 单独放 components 层会导致优先级错乱且两个动画分离在不同规则块，不是同一个可控整体。判定：这是对既有 legacy 规则的外科手术式追加，不是全新组件，留在 legacy 层、紧挨着 `stardrivePin` 是唯一正确选择（U30 规则本意是"新组件用新层"，不是"任何新增字符都要用新层"）。
  - **`animation-timeline` 从规格文字的匿名 `view()` 改成实际代码用的具名 `--stardriveScroll`**：`.stardrive.is-live` 已经用 `view-timeline-name:--stardriveScroll` 声明了具名时间线（`.stardrive` 200svh 外层包裹自身作为 timeline 主体，`.stardrive-stage` 引用它），规格文字草稿时没读到这层，写成了泛型 `view()`。改用真实的具名时间线 + `contain`/`exit` 命名区间关键字，与现有 pin 动画完全同源。
  - **两个动画同时作用于 `transform` 属性的合成问题**（规格文字没考虑到）：`stardrivePin` 的 `contain` 区间和 `stageShrink` 的 `exit` 区间前后相接不重叠，但 CSS 里同一属性被多个动画同时"生效"（含 forwards-fill 持续生效）时，列表里靠后的动画整体覆盖靠前的——若 `stageShrink` 的 `to` 只写 `scale(.94)` 不带 `translateY`，exit 阶段一开始舞台会瞬间跳回 `translateY(0)`（丢失 pin 已经推到底部的位移），造成真机验收要盯的那个交界帧炸裂。修正：`stageShrink` 的 `from`/`to` 都显式带上 `translateY(100svh)`，`to` 再叠加 `scale(.94)` 和 `clip-path`，确保交界帧位移连续，只有形状在变。
  - **字幕交棒（capFade）整段砍掉，不实现**：规格文字设想"字幕在 exit 阶段淡出，为下一章标题让位"，但读了 `alphardForge.js`（`progress()`/`render()`）才发现 `.stardrive-caption` 的透明度早就是 `opacity:calc(.18 + var(--forge)*.82)`——`--forge` 是 JS 每帧写入的滚动进度 0→1，字幕透明度设计上是**滚得越深越亮**（配合运镜 dolly-zoom 变近），而不是淡出。exit 阶段恰好是 `--forge` 逼近 1 的区间，此时若再叠加一个把 opacity 拉到 0 的新动画，会跟这条已经调过、真机验证过的既有效果正面打架，视觉上会抽搐。判定：新增字幕淡出与既有设计意图相反，不实现；"交棒"信号完全交给舞台本身的收缩+圆角+`clip-path`收窄来传达，够用。
  - 落地文件：`src/styles.css`（`@supports (animation-timeline: view())` 块内，`.stardrive-stage` 追加 `stageShrink` 关键帧）。
- [x] ⑯ 特性③：CSS 变量视差环（与 alphardForge 相机视差方向核对）。✅ 代码已完成（2026-07-18）：`src/main.js` 追加 `heroParallax()`（IIFE，~26 行）——passive `pointermove`（`pointerType!=='mouse'` 或 `!heroOn` 直接跳过）+ 自停 rAF lerp 写 `--mx`/`--my` 两个 CSS 变量到 `documentElement`；`IntersectionObserver` 盯 `.hero` 控制 `heroOn`；用仓库真实的 `REDUCED_MOTION` 常量（不是规格文字里占位写的 `RM`）在函数入口直接短路。`src/styles.css` 追加 `.hero-title/.hero-num/.hero-desc` 的 `@media(hover:hover)` 内 `transform:translate(calc(...))` 三层不同系数（深度感），`prefers-reduced-motion:reduce` 与 `(hover:none)` 一并清零。**与 alphardForge 视差的方向核对结论**：`.hero`（视差①）和 `.stardrive`（alphardForge 的相机视差）是滚动顺序上的两个独立区块，不会同屏出现，规格文字担心的"文字与星空反向漂移打架"实际上不构成真实冲突风险——仍沿用了 alphardForge 相同的 `clientX/innerWidth - 0.5` 居中换算方式保持手感一致，但不存在必须严格对齐符号的强约束，如实记录以免误导后续验收重点。⑮⑯合并验证：713/713 vitest，`vite build` 干净（沙盒挂载目录 `dist/.DS_Store` 环境问题同⑭，改临时 `--outDir` 验证），`!important` 计数 2504→2504 不变。
...
> | U42 | 新立项（2026-07-17）：Two Sigma 式交互数据星域——粒子即数据（坐标=阵营/置信度/类别三维语义），sectors 个股观察主落点，取代 U30 R3 力导向图；首页挂载为二期（待 U23 M2）；three/lerp/GLSL 全部现役零新依赖 | 未动工；切片① dataToSpace 纯函数可直接开工 |
>
