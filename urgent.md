...
5. **五行局 derivation**: spec §2.2 uses a year-stem × 命宫-branch lookup table; repo derives 局 from the 命宫 stem-branch's nayin element (五虎遁 + 纳音). **Verified 2026-07-23 during implementation: these are NOT two encodings of the same mapping — the spec's own table is wrong.** Full 10-stem × 12-branch cross-check against iztro (ground truth; same library ziwei.js's 400-chart comparison uses) shows the repo's nayin path matches iztro 120/120; the spec's §2.2 table disagrees with iztro (and the repo) on 8 of its 30 cells — the 午/未 and 戌/亥 columns are swapped relative to every stem-pair row (e.g. spec says 甲-year 午/未=Water2, 戌/亥=Earth5; iztro and the repo both give 午/未=Earth5, 戌/亥=Fire6). **Repo wins, confirmed, not assumed** — see `tests/ziweiBureauSpecTable.test.js` (121 passing: 120 iztro cross-checks + 1 test recording the specific spec discrepancy).
...
- [x] `npx vitest run` green — **1082/1082 passing** (up from 907 pre-Part-5). New suites: `baziSchema` (8), `horoscopeEngine` v2 additions (4 new, old invariants untouched), `synastryBazi` (8: A↔B symmetry, cell mirroring, hand-checked 子午冲 day×day case, missing-hour renormalization, harmony>clash ordering), `ziweiDeep` (27: 四化/aux-sha placements vs iztro, 大限 direction agrees with `dayunDirection()` over 20 random births + gapless 12-palace tiling, 三方四正 2x-opposing rule, sibling-palace rule, liunian palace, capped flowing-year scan), `deepSynthesis` (19: all 5 rules' agreement + crosscurrent branches, no-directive-language check), `ziweiBureauSpecTable` (121: full 10×12 iztro cross-check + the spec-table discrepancy finding below).
- [x] iztro spot-check harness extended: 四化 + all 12 辅星/煞星 placements cross-checked over **150 random births — 146/150 full-row matches**; the 4 mismatches are ALL leap-month births isolated to 左辅/右弼 specifically (iztro applies a mid-month day-split rule to these two stars in leap months; documented as a narrow, known school variance in `ziweiDeep.js`'s header, not a silent difference). 五行局 additionally cross-checked at full resolution: **120/120** across all 10 stems × 12 branches (see next item).
- [x] Spec-table reproduction: **finding, not a clean pass** — §2.2's 五行局 table does NOT reproduce from the repo's nayin path; cross-check against iztro shows the SPEC'S TABLE is wrong on 8/30 cells (午/未 ↔ 戌/亥 columns swapped), while the repo matches iztro 120/120 across all 10 stems × 12 branches. Repo kept as-is (already the iztro-verified path); discrepancy documented in §22.1 item 5 and `tests/ziweiBureauSpecTable.test.js`. `toChartMatrix()` output validated against spec §1.2 shape in `tests/ziweiDeep.test.js` (every star has `name/level/wuXing/transformation`).
- [x] Determinism audit: `grep -rn "Math.random" src/lib/` — zero hits in any fortune-math module (only in `transition.js`/`audio.js`/`sectorsGraphView.js`, all pre-existing visual/audio effects unrelated to this part); `dailyFortune`/`flowingYearScan`/`crossBranchMatrix` are pure functions of their inputs (no `Date.now()`/`Math.random()` anywhere in the call path) — same (birth, date) always → byte-identical output.
- [x] `npm run build` + `node scripts/check-bundle-budget.mjs` green — `horoscope` chunk 206.8 kB / 300 kB budget (69%, up from 185.3 kB pre-Part-5 — all growth is table math + new UI, no new deps); `main` unaffected at 187.4 kB / 250 kB. `npx tsc --noEmit` clean throughout.
- [x] Copy audit (§22.4): every new user-facing string checked for verdict/health/financial-directive language — clean (the only "prohibited"/"DO NOT" hits are in module-header comments explicitly contrasting the site's actual copy against the spec's rejected phrasing, never rendered to users). Every new `data-en` has a matching `data-zh`. Beta badges present on all three new surfaces: daily-fortune panel, 合盘 branch matrix, ZWDS deep layer. Entertainment-only caveat paragraph present under each.
- [x] design.md compliance, **corrected from this section's original assumption**: horoscope.html does NOT use arena's `--acid`/`--cyan`/`--magenta` viz tokens (design.md §1.3 confirms horoscope's own independent palette: 暖奶油/鼠尾草绿/赤陶橙/淡金, `--sage-deep`/`--terra-deep`/`--gold-deep`). All new UI (Beta badges, branch-matrix cells, palace warning borders, synthesis cards) was built against horoscope's OWN existing tokens instead — no `:root` edits, no new fonts, no cross-page token borrowing. `git diff public/styles/horoscope.css` is 75 net-new lines, zero edits to existing rules.
- [x] Frozen-file audit: `git diff --stat src/lib/ziwei.js src/lib/bazi.js src/lib/lunar.js` — **empty, zero changes**. `git diff src/lib/ziping.js src/lib/dayun.js` — every changed line is an added `export` keyword or its explanatory comment; zero logic changes (verified line-by-line, also implied by all pre-existing test suites for these files passing unchanged).
...
# Part 3 (NEW 2026-07-24) — Ambient Music Player for serial.html (U-ambient-player)
