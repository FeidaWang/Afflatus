...
| CV-P0-07 | P0 | M | **In progress:** production manifests require same-package landmark admission and at least five frozen canonical views. Registry/CI re-hashes the admission JSON, every LOD GLB and every desktop/mobile night golden; it also requires the exact frozen AOI/origin, recomputes each camera's horizontal ENU pose from WGS84 and rejects >5 cm drift or wrong order/FOV. Silhouette-mask and performance-trace goldens remain | A package cannot enter the public registry until every gate and four-role approval is green |
