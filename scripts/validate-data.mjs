...
if (existsSync(CITY_PACKAGE_REGISTRY_PATH) && existsSync(CITY_LEDGER_PATH)) {
  try {
    const registry = JSON.parse(readFileSync(CITY_PACKAGE_REGISTRY_PATH, 'utf8'));
    const ledger = JSON.parse(readFileSync(CITY_LEDGER_PATH, 'utf8'));
    ...
    const { ok, errors } = validateCityPackageReleaseReferences(registry, packagesByPath, ledger);
    checked++;
    if (!ok) {
