...
def audit():
    print("=== STARTING AUDIT FOR BATCH 32 (CHAPTERS 156 - 160) ===")
    ...
    batch_indices = range(155, 160)
    batch_chapters = [chapters[i] for i in batch_indices]

    errors = []

    # 1. Check word counts for Batch 32
    ...
    # 2. Check forbidden words in Batch 32
    print("\n--- 2. Checking Forbidden Words in Batch 32 ---")
    for idx in batch_indices:
        ch = chapters[idx]
    ...
    if not any("forbidden word" in e for e in errors):
        print("PASS: 0 forbidden words found in Batch 32!")

    # 3. Cross-chapter duplicate check for Batch 32 against all 200 chapters
    print("\n--- 3. Cross-chapter Duplicate Check for Batch 32 against ALL 200 Chapters ---")
    batch32_dups = []
    for idx in batch_indices:
        ch = chapters[idx]
        ...
        for b_idx, blk in enumerate(ch.get("blocks", [])):
            ...
            for b2_idx, blk2 in enumerate(ch.get("blocks", [])):
                if b2_idx > b_idx and blk2.get("text", "").strip() == t:
                    batch32_dups.append((cid, b_idx, cid, b2_idx, t[:30]))
            ...
            for o_idx, other_ch in enumerate(chapters):
                ...
                for ob_idx, oblk in enumerate(other_ch.get("blocks", [])):
                    ...
                    if t == ot:
                        batch32_dups.append((cid, b_idx, other_ch["id"], ob_idx, t[:30]))

    if batch32_dups:
        print(f"FAIL: Found {len(batch32_dups)} duplicate blocks for Batch 32!")
        for d in batch32_dups[:10]:
            print(f"  Ch {d[0]}[{d[1]}] duplicates with Ch {d[2]}[{d[3]}]: '{d[4]}...'")
            errors.append(f"Duplicate block between Ch {d[0]} and Ch {d[2]}")
    else:
        print("PASS: 0 duplicate blocks for Batch 32 across all 200 chapters!")
    ...
    if errors:
        ...
        print("ALL CHECKS PASSED PERFECTLY FOR BATCH 32!")
