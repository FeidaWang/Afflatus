...
describe('buildProvenanceBadge', () => {
  it('seed state (updatedAt null) — en', () => {
    ...
    expect(b.text).toBe('FABLE 5 MAX · 5.6 SOL ULTRA · no data yet · v1 · NOT ADVICE');
  });

  it('seed state — zh', () => {
    const b = buildProvenanceBadge({ updatedAt: null, version: 1, lang: 'zh' });
    expect(b.tier).toBe('seed');
    expect(b.text).toBe('FABLE 5 MAX · 5.6 SOL ULTRA · 暂无数据 · v1 · 非投资建议');
  });
  ...
  it('fresh state with sourceCount and version — en', () => {
    ...
    expect(b.text).toBe('FABLE 5 MAX · 5.6 SOL ULTRA · 6h ago · 5 sources · v3 · NOT ADVICE');
  });
  ...
  it('amber state — zh, no sourceCount', () => {
    ...
    expect(b.text).toBe('FABLE 5 MAX · 5.6 SOL ULTRA · 2 天前 · v2 · 非投资建议');
  });
  ...
  it('red state — omits version when not provided', () => {
    ...
    expect(b.text).toBe('FABLE 5 MAX · 5.6 SOL ULTRA · 5d ago · NOT ADVICE');
  });
