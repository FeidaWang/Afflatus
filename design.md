...
专项研究（待实施）：[Astra 动效、图表与交互借鉴手册](docs/astra-motion-design.md)——2026-09-05 实站观察，含全站页面映射、17 个优先级模块、28 条 Codex prompts 和截图证据；属于设计提案，不表示这些改版已上线。
...
- **系统播报面板（U-sys-panel）**：小说内系统提示（`type:"sys"` 块）渲染为「跨界游戏终端」卡片——冷青绿低蓝光「屏幕」对暖色衬线正文，mono 字体 + 扫描线 + 虚线内框 + 标题芯片（正文首个`【…】`解析为面板标题）+ 呼吸态链接点（`prefers-reduced-motion` 下停用）。护眼实测：日间 ink/bg 9.8:1、accent 7.1:1；夜间 11.7:1 / 9.8:1。色值随 `.reader[data-theme]` 走 `--sys-*` token，重构时整体保留。
