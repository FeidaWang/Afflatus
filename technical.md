...
```
> **V20 观星台（horoscope.html，2026-07-05）架构要点**：整页零 fetch 零后端——四柱排盘/星座/日运/合盘全部在浏览器本地由上述两个纯函数库算出；生日只存 localStorage（`afflatus-horo:me`），签到 streak 同理；合盘分享 = 双方生日 base64url 编进 `?p=` 参数（`encodeShare`/`decodeShare`，有 vitest 覆盖的输入校验）。日运的「每天变化」来自真实干支历（今日日柱五行 vs 用户日主生克）+ seeded mulberry32 文案选择，确定性、可复现、无 API 成本。**节气/立春边界（2026-07-05 修正）**：不再用固定日期近似，改为 Meeus 低精度太阳视黄经公式 + 二分法实时算出每年真实节气日期（换算北京时间），修了「2025 年立春实际 2/3 而非惯常 2/4」这类固定表会判错的边界案例，测试用公开发布的真实节气时刻做夹具验证；西方星座分界日仍用通用近似表（传统上如此，非本次范围）。**出生地时区/夏令时（2026-07-05 新增）**：`normalizeBirthToCST()` 在表单提交时把任意时区+夏令时的出生时间一次性换算到北京时间框架（含中国自己 1986-1991 历史夏令时的自动修正），换算后的规范值才存档/进分享链接，下游 `computeBazi`/`encodeShare` 完全不用感知时区概念。**合盘双盘对照（2026-07-05 新增）**：`synastry()` 早就返回 `chartA`/`chartB`，此前从未渲染，现在左右横向并置显示。**美术**：主题色改爱马仕橙 `#F37022` + 24k 金 `#EFBF04`（用户提供爱马仕龙马烟花壁纸后精确到这两个色值），背景新增手绘 SVG 奔马+龙纹水墨线描（呼应 2026 马年），另在 Hero 区加了一版更显眼的彩色插画（月白龙 `--dragon:#efe3c8` + 枣红马 `--horse:#8a3020`，参照壁纸配色，opacity .4，马朝向翻转面对龙），画完都用 `cairosvg` 渲图核对形状/合成效果可辨认后才定稿。**龙姿势迭代（2026-07-06）**：用户坚持要求参照壁纸里单独抠出的龙，重画为盘坐/蜷曲姿势（原为水平奔跑波浪身形），前腿加肘部弯折+三趾爪、尾巴独立打卷、脊背加一排小三角鳍片（区分"龙"与"蠕虫/恐龙"轮廓的关键视觉标志，`cairosvg` 多轮迭代后才补上）；Hero 插画宽度上限收窄（`56vw/520px`→`48vw/440px`）给正文留安全边距。全页水印版龙保留 V20.6 加腿版，未跟进鳍片（透明度仅 .08，细节不可见）。**晚子时换日（2026-07-05 修正）**：用户拿专业排盘网站交叉验证发现本站 23 点出生的日柱/时柱和主流站不一致——根因是「早晚子时是否换日」的历法惯例选择，`computeBazi()` 现在对 hour=23 的出生按次日重算年/月/日柱（时柱分支仍是子，天干用次日日干起），用真实案例（1992-02-23 23:26）逐柱核对与参考站完全吻合；「子」时辰选项也拆成早子(00:xx)/晚子(23:xx)两个，避免二者被混成同一个 hour 值。全页挂「仅供娱乐」。**生辰详批扩展（2026-07-06）**：新增 `src/lib/ziping.js`（子平法分析层，独立于 `bazi.js` 的历法计算），提供十神(算法推导,不查表)/藏干/纳音/空亡/十二长生(地势+自坐)/旺相休囚死/天干地支刑冲合害/21个神煞(驿马/华盖/将星/桃花/劫煞/灾煞/岁煞/孤辰/寡宿/禄神/羊刃/天乙贵人/文昌贵人/学堂/福星贵人/金舆/国印贵人/月德贵人/天德贵人/魁罡/童子)/简化"扶抑法"身强身弱+格局+喜用神忌神+0-100娱乐向评分。所有表格用用户提供的真实排盘截图（1992-02-23 23:26→壬申壬寅庚午丙子）反推校验，10个神煞+全部藏干/纳音/空亡/地势/自坐/刑冲合害/旺相休囚死直接对照验证通过，另11个未在截图中出现示例的神煞采用标准文献表且代码注释如实标注未验证；童子煞额外注明流派分歧。`horoscope.html` 新增"生辰详批"表格区（`.bz-*` CSS），由 `horoscope.js` 的 `baziDetailHTML()` 渲染。**V21 Phase 0 治愈系改版（2026-07-06）**：页面视觉从水墨松烟夜色全面转为暖奶油/鼠尾草绿/赤陶橙/淡金（`public/styles/horoscope.css` 全量重写，类名不变；文字色一律用深色变体，WCAG AA 对最深底逐一验证）；删除水墨底板与龙马美术，换原创植物线稿 SVG；`page-turn.css` 的 `.horoscope-page` 作用域块同步换色；新增 `src/lib/shareCard.js`（canvas PNG 分享卡，DOM 耦合不补 vitest）。**V21 Phase 1（2026-07-06）**：`tenGodDistribution()`（加权藏干十神占比）+ `tiaohou` 简化调候（仅冬宜火/夏宜水两档）+ 神煞稀有度——`scripts/gen-shensha-rarity.mjs` 枚举 14.5 万均匀合成命盘统计频率，产物为生成的进-git 模块 `src/lib/shenshaRarity.js`（保持页面零 fetch），魁罡频率与理论值 4/60 对照通过。**V21 Phase 2（2026-07-06）**：`src/lib/dayun.js`（大运排向/起运天数折岁/十年一柱序列/流年干支/值冲刑害破太岁五档/流年与命局关系），依赖 `bazi.js` 新导出的 `adjacentSolarTerms()`（真实节气瞬间）；性别字段只进 localStorage 档案、不进分享码（决策见 RELEASE_NOTES，规避 schema 版本兼容）。**V21 Phase 3（2026-07-06）**：`src/lib/lunar.js`（经典 1900–2100 紧凑表+自写转换，与独立实现 10,815 天对照零不一致；表数据经 npm 渠道从 MIT `solarlunar` 包提取——沙盒 web 抓取被拦截时 npm 是可用替代通道）+ `src/lib/xiu.js`（值日 28 宿：锚点对照独立库+星期锁验证；本命 27 宿：宿曜经月首宿表，无独立源可对照已标注；三九秘法双人关系）。**V21 Phase 4–6（2026-07-06，V21 全部完成）**：`persona.js`（24 题自编十六型速测+纯计分，商标免责声明在页，C5 评估后页内分区不开新页）；`astro.js`（日月升：月亮 Meeus 截断级数以农历初一距角穿零做物理验证、上升公式双不变量校准；行星缓做——VSOP87 表不可凭记忆手打/星历库+350KB 待决策）；`ziwei.js`（紫微入门版：十二宫+十四主星完整安星，与 iztro 400 盘对照全对；年柱按农历年惯例、辅星四化留待后续）。horoscope 页 chunk 最终 73.3KB（V20 的 24.6KB → 三倍，仍在合理页面级 chunk 范围）。
