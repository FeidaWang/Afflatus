...
| `CITY-REAL-P0-07` | 自动门禁推进完成（实际证据/实体签署未完成） | 2026-09-05 City Chromium 回归 `16/16`、active metadata/keyboard/Axe `3/3`、Vitest `1930/1930`、production build、bundle budget 与 57 份数据证据通过；schema-v2 admission 已强制 silhouette/performance evidence；三轮 Lighthouse performance 均 `1.00`，中位 FCP/LCP/Speed Index `1352.74 ms`、TBT `0 ms`、CLS `0.02` | 当前 registry 为空，以上不构成真实包签署；仍需捕获真实包证据、完成人工无障碍与实体 iPhone/Samsung GPU/热/手势签署 |
...
10. `CITY-REAL-P0-07` — **自动门禁已补齐，实际证据与实体签署待办**：schema-v2 landmark admission 现强制同包 canonical desktop/mobile silhouette masks 与两份至少 30 分钟的结构化 performance traces，并逐项检查相机 × day/twilight/night 覆盖、WebGL、draw-call/triangle/p95/GPU/横向溢出预算、bytes/hash；Chromium、Axe、成功/poster 视觉、既有 30 分钟长稳、三轮 Lighthouse、production build 与数据门禁已通过。下一步仍须捕获真实包证据，完成人工无障碍与 iPhone/Samsung 实体真机 GPU/热/手势签署；当前 registry 为空，自动结果不构成真实包发布批准。
