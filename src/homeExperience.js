...
let marketDeckPromise=null;
function loadMarketDeck(){
  if(marketDeckPromise) return marketDeckPromise;
  marketDeckPromise=import('./ui/marketDeck.js').then(({initMarketDeck})=>{
    marketDeck=initMarketDeck({
      getLang:()=>currentLang,
      onPickHotChange:on=>cursorCtl.setHot(on)
    });
    marketDeck.updatePeriodUI();
    marketDeck.renderPicks(COPY[currentLang].picks);
    return marketDeck;
  });
  return marketDeckPromise;
}
function scheduleMarketDeck(){
  const targets=['fy2026Performance','portfolioConvoy']
    .map(id=>document.getElementById(id))
    .filter(Boolean);
  if(!targets.length) return;
  if(!('IntersectionObserver' in window)){
    setTimeout(()=>{void loadMarketDeck().catch(()=>{});},800);
    return;
  }
  const observer=new IntersectionObserver((entries)=>{
    if(!entries.some(entry=>entry.isIntersecting)) return;
    observer.disconnect();
    void loadMarketDeck().catch(()=>{});
...
function loadVoyageLog(){
  if(voyageLogPromise) return voyageLogPromise;
  voyageLogPromise=import('./ui/voyageLogConsole.js').then(({initVoyageLogConsole})=>{
  ...
  return voyageLogPromise;
}

(function installVoyageLogIntent(){
  const toggle=document.getElementById('voyageLogToggle');
  if(!toggle) return;
  const warm=()=>{void loadVoyageLog().catch(()=>{});};
  toggle.addEventListener('pointerover',warm,{passive:true});
  toggle.addEventListener('focus',warm);
  toggle.addEventListener('click',async(event)=>{
    if(voyageLogCtl) return;
    event.preventDefault();
