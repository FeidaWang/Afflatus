...
(() => {
  ...
  /* ---------- Fable track record — shared with games.js via
     src/lib/trackRecord.js (ROADMAP §7.5 V12) ---------- */
  function renderRecord() {
    const host = $('record'); if (!host) return;
    const r = data && data.record;
    const html = renderTrackRecordHTML(r, { T, fableIcon: FABLE_ICON, exactLabel: { en: 'exact series score', zh: '比分全中' } });
  ...
  fetchJson('leagues').then((d) => { data = d; renderAll(); }).catch(() => { if ($('series')) $('series').innerHTML = `<div class="empty">${T('Series data unavailable.', '赛程数据暂不可用。')}</div>`; });
