...
(() => {
  ...
  function fmtAxisTime(t, tf) { if (!t || t < 1e11) return ''; const d = new Date(t), p2 = (x) => String(x).padStart(2, '0'), md = `${p2(d.getMonth() + 1)}-${p2(d.getDate())}`; return tf === 'D' ? `${p2(d.getHours())}:${p2(d.getMinutes())}` : tf === 'W' ? `${md} ${p2(d.getHours())}:${p2(d.getMinutes())}` : `${d.getFullYear()}-${md}`; }
  ...
  function renderChart() {
    ...
    if (showMA) { if (ma7) s += `<path d="${mapLineY(ma7, n, X, Y)}" class="cma cma7" fill="none"/>`; if (ma30) s += `<path d="${mapLineY(ma30, n, X, Y)}" class="cma cma30" fill="none"/>`; s += `<text x="${(plotW - 3).toFixed(1)}" y="11" class="cma-lg" text-anchor="end"><tspan class="cma7">MA7</tspan> <tspan class="cma30">MA30</tspan></text>`; }
    ...
    const ticks = Math.min(5, n); let lastLab = ''; for (let k = 0; k < ticks; k++) { const i = Math.round(k / Math.max(1, ticks - 1) * (n - 1)), lab = fmtAxisTime(times[i], tf); if (!lab || lab === lastLab) continue; lastLab = lab; const anchor = k === 0 ? 'start' : (k === ticks - 1 ? 'end' : 'middle'); s += `<text x="${X(i).toFixed(1)}" y="${(Htotal - 4).toFixed(1)}" class="caxis-x" text-anchor="${anchor}">${lab}</text>`; }
    s += `<g id="xhair"></g>`;
  ...
  function renderPred() {
    ...
    const modeTabs = `<div class="pmode" role="tablist" aria-label="${T('Prediction type', '预测类型')}"><button class="pmode-b ${state.predMode === 'OPEN' ? 'on' : ''}" data-pm="OPEN" role="tab" aria-selected="${state.predMode === 'OPEN'}"><b>${T('OPEN', '开盘')}</b><i>${T('at 9:30 ET', '09:30 开盘价')}</i></button><button class="pmode-b ${state.predMode === 'CLOSE' ? 'on' : ''}" data-pm="CLOSE" role="tab" aria-selected="${state.predMode === 'CLOSE'}"><b>${T('CLOSE', '收盘')}</b><i>${T('at 4:00 ET', '16:00 收盘价')}</i></button></div>`;
