...
(() => {
  ...
  function pickCard(model, p, stale) {
  ...
  function render() {
    ...
    const freshness = assessMarketSnapshot(d.date, new Date(), { availableFromMinutes: ARENA_PUBLICATION_MINUTES.picks });
    const stale = freshness.stale;
    $('picksNote').textContent = stale
      ? T(
        `Historical snapshot · last published ${d.date || 'unknown'}. Automation is delayed; these are not current recommendations.`,
        `历史快照 · 最后发布于 ${d.date || '日期不明'}。自动任务当前延迟；以下不是当前推荐。`,
      )
