...
export function createOdinHull(THREE, { add, mats, detail = 'full' }) {
  ...
  add(new THREE.ConeGeometry(1.05, BOW_LEN, 4), M.hull, [0, 0, bowMid], [Math.PI / 2, 0, Math.PI / 4], [1, HEIGHT * 0.13, 1]);
  add(new THREE.BoxGeometry(1.5, HEIGHT * 0.13, BOW_LEN * 0.5), M.arm, [0, HEIGHT * 0.07, BOW_ROOT + BOW_LEN * 0.28]);
  for (let i = 0; i < 4; i++) add(new THREE.BoxGeometry(1.3 - i * 0.24, 0.03, 0.14), M.trim, [0, HEIGHT * 0.1, NOSE - 0.5 - i * (BOW_LEN * 0.2)]);
  ...
  const mastBaseY = HEIGHT * 0.82, mastBaseZ = tierZ;
  const MASTS = [
  ...
  for (const sx of [-1, 1]) {
    const px = sx * 0.85, py = -HEIGHT * 0.34, pz = midMid - midLen * 0.1;
  ...
  add(new THREE.BoxGeometry(2.3, HEIGHT * 0.5, STERN_ROOT - STERN), M.arm, [0, 0, sternMid]); // engine deck block
  const engineMounts = [
    { x: -0.7, y: 0.14 }, { x: 0.7, y: 0.14 },
    { x: -0.7, y: -0.28 }, { x: 0.7, y: -0.28 },
    { x: -0.32, y: -0.5 }, { x: 0.32, y: -0.5 },
    { x: 0, y: 0.42 },
  ].map(m => ({ x: m.x, y: m.y * HEIGHT * 0.7, z: STERN + 0.35 }));
