...
export function createCitySandbox({
  canvas,
  plan,
  initialDay = 0,
  ...
  if (!canvas) throw new Error('City sandbox requires a canvas.');
  if (!plan) throw new Error('City sandbox requires a generated plan.');
  ...
  budgetSurface = coordinator.register({
    ...
    observe: true,
    cost: 'high',
    targetFps: CITY_SANDBOX_RENDER_BUDGET[budgetClass].targetFps,
    onResume: () => {
