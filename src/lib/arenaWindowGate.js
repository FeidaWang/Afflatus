import { easternTimeParts, isNyseSession, isEarlyCloseSession, hasVerifiedNyseCalendar } from './marketSession.js';
...
export { isEarlyCloseSession } from './marketSession.js';
...
export function assessArenaWindow(windowName, value = new Date(), options = {}) {
  ...
  if (!hasVerifiedNyseCalendar(date)) reason = 'calendar-unverified';
  else if (!session) reason = 'not-nyse-session';
