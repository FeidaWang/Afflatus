...
(() => {
  ...
  // ---------- run + intercept ----------
  let busy = false;
  function run(href, dir) { if (busy) return; busy = true; const type = TYPE(new URL(href, location.href).pathname); try { sfx(type); } catch {} const dur = RM ? 160 : 720; if (!RM) { try { animate(type, dur); document.body.classList.add(dir === 'prev' ? 'turn-prev' : 'turn-next'); } catch {} } setTimeout(() => { try { document.documentElement.classList.add('vt-suppress'); } catch {} window.location.href = href; }, dur); }
  window.AfflatusFX = { run, sfx, type: TYPE };
  function internal(a) {
    if (!a || a.target === '_blank' || a.hasAttribute('download')) return null;
    // Locale controls own their navigation because they must preserve the
    // current query string and hash. The capture-phase page transition
    // interceptor runs before their click handlers, so explicitly yield here.
    if (a.matches('.lang-toggle, #langBtn, #langMiniToggle')) return null;
    ...
    if (!(u.pathname === '/' || /^\/(?:en|zh)\/$/.test(u.pathname) || /\.html?$/.test(u.pathname))) return null;
