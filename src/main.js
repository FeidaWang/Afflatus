...
const shipSideRenderer=null;
const shipRearRenderer=null;
...
function createShipRenderer(canvas,mode='side'){
  if(!canvas) return null;
  ...
  const frag=`
    ...
    uniform float uIntensity;
    uniform float uMode;
    uniform float uCannon;

    float box(vec2 p,vec2 c,vec2 b){vec2 d=abs(p-c)-b;return 1.0-smoothstep(0.0,0.010,max(d.x,d.y));}
    float ellipse(vec2 p,vec2 c,vec2 r){return 1.0-smoothstep(1.0,1.035,length((p-c)/r));}
    float seg(vec2 p,vec2 a,vec2 b,float w){vec2 pa=p-a,ba=b-a;float h=clamp(dot(pa,ba)/dot(ba,ba),0.0,1.0);return 1.0-smoothstep(w,w+0.003,length(pa-ba*h));}
    float tri(vec2 p,vec2 a,vec2 b,vec2 c){
      vec2 e0=b-a,e1=c-b,e2=a-c;
      float s0=sign(e0.x*(p.y-a.y)-e0.y*(p.x-a.x));
      float s1=sign(e1.x*(p.y-b.y)-e1.y*(p.x-b.x));
      float s2=sign(e2.x*(p.y-c.y)-e2.y*(p.x-c.x));
      return (s0==s1 && s1==s2)?1.0:0.0;
    }
    vec3 steel(float l){return mix(vec3(0.035,0.055,0.075),vec3(0.62,0.70,0.76),l);}
    void main(){
      ...
      if(uMode<0.5){
        float upperHull=tri(p,vec2(0.035,0.570),vec2(0.885,0.335),vec2(0.955,0.500));
        float lowerHull=tri(p,vec2(0.035,0.570),vec2(0.910,0.505),vec2(0.800,0.800));
        float dorsalPlane=tri(p,vec2(0.175,0.565),vec2(0.865,0.395),vec2(0.890,0.505));
        float ventralPlane=tri(p,vec2(0.160,0.596),vec2(0.810,0.775),vec2(0.905,0.505));
        float hull=max(max(upperHull,lowerHull),max(dorsalPlane*.52,ventralPlane*.48));
        float shade=0.34+0.34*p.x+0.30*(1.0-p.y);
        color+=steel(shade)*hull;
        color+=vec3(0.015,0.045,0.070)*ventralPlane*.18;
        alpha=max(alpha,hull*.95);

        float shadowBand=seg(p,vec2(0.095,0.642),vec2(0.900,0.535),0.016);
        color=mix(color,vec3(0.006,0.014,0.024),shadowBand*.52);
        alpha=max(alpha,shadowBand*.36);

        float deck=0.0;
        deck+=box(p,vec2(0.565,0.367),vec2(0.230,0.026));
        deck+=box(p,vec2(0.610,0.320),vec2(0.185,0.022));
        deck+=box(p,vec2(0.650,0.277),vec2(0.138,0.020));
        deck+=box(p,vec2(0.682,0.232),vec2(0.084,0.021));
        deck+=box(p,vec2(0.707,0.185),vec2(0.048,0.050));
        deck+=box(p,vec2(0.710,0.130),vec2(0.108,0.027));
        deck+=box(p,vec2(0.620,0.152),vec2(0.022,0.031));
        deck+=box(p,vec2(0.800,0.152),vec2(0.022,0.031));
        deck+=box(p,vec2(0.710,0.088),vec2(0.018,0.044));
        color+=steel(0.56+0.22*p.x)*deck;
        alpha=max(alpha,deck*.95);

        float edges=0.0;
        edges+=seg(p,vec2(0.035,0.570),vec2(0.885,0.335),0.0030);
        edges+=seg(p,vec2(0.035,0.570),vec2(0.800,0.800),0.0030);
        edges+=seg(p,vec2(0.885,0.335),vec2(0.955,0.500),0.0028);
        edges+=seg(p,vec2(0.910,0.505),vec2(0.800,0.800),0.0028);
        edges+=seg(p,vec2(0.108,0.575),vec2(0.885,0.500),0.0021);
        edges+=seg(p,vec2(0.225,0.640),vec2(0.830,0.530),0.0017);
        edges+=seg(p,vec2(0.305,0.510),vec2(0.820,0.405),0.0016);
        edges+=seg(p,vec2(0.365,0.696),vec2(0.825,0.562),0.0016);
        for(int i=0;i<20;i++){
          float t=float(i)/19.0;
          vec2 a=mix(vec2(0.120,0.574),vec2(0.826,0.402),t);
          vec2 b=mix(vec2(0.230,0.735),vec2(0.864,0.552),t);
          edges+=seg(p,a,b,0.00115);
        }
        for(int i=0;i<18;i++){
          float t=float(i)/17.0;
          float x=0.195+t*.610;
          edges+=box(p,vec2(x,0.533+0.024*sin(t*3.14159)),vec2(0.017,0.0032));
          edges+=box(p,vec2(x+.018,0.622-0.022*sin(t*3.14159)),vec2(0.018,0.0032));
          edges+=box(p,vec2(x+.035,0.470+0.018*sin(t*6.28318)),vec2(0.010,0.0025));
        }
        for(int i=0;i<9;i++){
          float t=float(i)/8.0;
          edges+=seg(p,vec2(0.430+t*.330,0.390),vec2(0.452+t*.285,0.347),0.0012);
          edges+=seg(p,vec2(0.460+t*.260,0.322),vec2(0.475+t*.220,0.294),0.0011);
        }
        color+=vec3(0.76,0.95,1.0)*edges*.33;
        alpha=max(alpha,clamp(edges*.48,0.0,.78));

        float turrets=0.0;
        for(int i=0;i<9;i++){
          float t=float(i)/8.0;
          turrets+=box(p,vec2(0.465+t*.300,0.446+0.020*sin(t*3.14159)),vec2(0.008,0.006));
          turrets+=seg(p,vec2(0.465+t*.300,0.438+0.020*sin(t*3.14159)),vec2(0.452+t*.300,0.424+0.018*sin(t*3.14159)),0.0012);
        }
        color+=vec3(0.82,0.97,1.0)*turrets*.42;
        alpha=max(alpha,turrets*.42);

        float engines=max(ellipse(p,vec2(0.895,0.475),vec2(0.034,0.030)),max(ellipse(p,vec2(0.920,0.565),vec2(0.052,0.046)),ellipse(p,vec2(0.890,0.662),vec2(0.036,0.034))));
        engines=max(engines,max(ellipse(p,vec2(0.852,0.520),vec2(0.022,0.023)),ellipse(p,vec2(0.852,0.610),vec2(0.022,0.023))));
        color=mix(color,vec3(0.005,0.014,0.028),engines*.82);
        alpha=max(alpha,engines*.95);
        float pulse=0.80+0.20*sin(uTime*5.1);
        float flame=(ellipse(p,vec2(0.977,0.475),vec2(0.070+0.012*sin(uTime*5.0),0.021))+
                    ellipse(p,vec2(1.000,0.565),vec2(0.116+0.022*sin(uTime*4.7),0.034))+
                    ellipse(p,vec2(0.977,0.662),vec2(0.074+0.014*sin(uTime*5.5),0.022))+
                    ellipse(p,vec2(0.932,0.520),vec2(0.044,0.015))+
                    ellipse(p,vec2(0.932,0.610),vec2(0.044,0.015)))*(0.54+uIntensity*.82)*pulse;
        color+=mix(vec3(0.13,0.92,1.0),vec3(0.50,0.34,1.0),uIntensity)*flame;
        alpha=max(alpha,clamp(flame*.70,0.0,.96));

        float muzzle=box(p,vec2(0.058,0.570),vec2(0.042,0.006))*uCannon;
        float beam=seg(p,vec2(0.045,0.570),vec2(-0.56,0.470),0.024)*uCannon;
        color+=vec3(1.0,0.16,0.30)*beam+vec3(1.0,0.88,0.82)*muzzle;
        alpha=max(alpha,beam*.95+muzzle*.86);
      }else{
        float rearFace=tri(p,vec2(0.050,0.745),vec2(0.500,0.270),vec2(0.950,0.745));
        rearFace=max(rearFace,tri(p,vec2(0.050,0.745),vec2(0.500,0.935),vec2(0.950,0.745)));
        float upperPlate=tri(p,vec2(0.145,0.610),vec2(0.500,0.310),vec2(0.855,0.610));
        float lowerKeel=tri(p,vec2(0.150,0.760),vec2(0.500,0.918),vec2(0.850,0.760));
        float body=max(rearFace,max(upperPlate*.58,lowerKeel*.50));
        color+=steel(0.50+0.26*(1.0-p.y))*body;
        alpha=max(alpha,body*.95);

        float tower=0.0;
        tower+=box(p,vec2(0.500,0.260),vec2(0.170,0.052));
        tower+=box(p,vec2(0.500,0.194),vec2(0.122,0.030));
        tower+=box(p,vec2(0.390,0.218),vec2(0.028,0.043));
        tower+=box(p,vec2(0.610,0.218),vec2(0.028,0.043));
        tower+=box(p,vec2(0.500,0.132),vec2(0.018,0.070));
        tower+=box(p,vec2(0.500,0.085),vec2(0.118,0.012));
        color+=steel(0.63)*tower;
        alpha=max(alpha,tower*.95);

        float edges=0.0;
        edges+=seg(p,vec2(0.050,0.745),vec2(0.500,0.270),0.003);
        edges+=seg(p,vec2(0.950,0.745),vec2(0.500,0.270),0.003);
        edges+=seg(p,vec2(0.050,0.745),vec2(0.500,0.935),0.003);
        edges+=seg(p,vec2(0.950,0.745),vec2(0.500,0.935),0.003);
        edges+=seg(p,vec2(0.168,0.616),vec2(0.832,0.616),0.002);
        edges+=seg(p,vec2(0.120,0.744),vec2(0.880,0.744),0.002);
        for(int i=0;i<17;i++){
          float t=float(i)/16.0;
          float x=0.105+t*.790;
          edges+=seg(p,vec2(x,0.735),vec2(mix(x,0.5,.48),0.322),0.00125);
        }
        for(int i=0;i<9;i++){
          float y=0.405+float(i)*0.052;
          edges+=seg(p,vec2(0.145,y),vec2(0.855,y),0.0013);
        }
        for(int i=0;i<16;i++){
          float t=float(i)/15.0;
          float x=0.210+t*.580;
          edges+=box(p,vec2(x,0.640+0.030*sin(t*3.14159)),vec2(0.018,0.004));
          edges+=box(p,vec2(x,0.705-0.018*sin(t*3.14159)),vec2(0.014,0.003));
        }
        edges+=seg(p,vec2(0.240,0.570),vec2(0.760,0.570),0.0014);
        edges+=seg(p,vec2(0.305,0.520),vec2(0.695,0.520),0.0012);
        color+=vec3(0.78,0.96,1.0)*edges*.33;
        alpha=max(alpha,edges*.48);

        float shell=max(ellipse(p,vec2(0.285,0.700),vec2(0.065,0.075)),max(ellipse(p,vec2(0.500,0.735),vec2(0.096,0.113)),ellipse(p,vec2(0.715,0.700),vec2(0.065,0.075))));
        shell=max(shell,max(ellipse(p,vec2(0.390,0.593),vec2(0.038,0.045)),ellipse(p,vec2(0.610,0.593),vec2(0.038,0.045))));
        shell=max(shell,max(ellipse(p,vec2(0.335,0.820),vec2(0.036,0.044)),ellipse(p,vec2(0.665,0.820),vec2(0.036,0.044))));
        color=mix(color,vec3(0.005,0.014,0.030),shell*.74);
        alpha=max(alpha,shell*.95);

        float pulse=0.78+0.22*sin(uTime*5.2);
        float fire=max(ellipse(p,vec2(0.285,0.792),vec2(0.090,0.108)),max(ellipse(p,vec2(0.500,0.860),vec2(0.136,0.162)),ellipse(p,vec2(0.715,0.792),vec2(0.090,0.108))));
        fire=max(fire,max(ellipse(p,vec2(0.390,0.652),vec2(0.049,0.060)),ellipse(p,vec2(0.610,0.652),vec2(0.049,0.060))));
        fire=max(fire,max(ellipse(p,vec2(0.335,0.872),vec2(0.048,0.056)),ellipse(p,vec2(0.665,0.872),vec2(0.048,0.056))));
        vec3 drive=mix(vec3(0.12,0.94,1.0),vec3(0.50,0.35,1.0),uIntensity);
        color+=drive*fire*(pulse+uIntensity*.82);
        alpha=max(alpha,clamp(fire*.84,0.0,.96));
      }

      vec2 fpos=mix(vec2(0.20,0.22),vec2(0.82,0.24),step(0.5,uMode));
      vec2 q=(p-fpos)/vec2(0.105,0.072);
      float f47=0.0;
      f47+=tri(q,vec2(-1.18,0.0),vec2(0.72,-0.22),vec2(0.72,0.22));
      f47+=tri(q,vec2(-0.18,0.0),vec2(0.54,-0.84),vec2(0.38,-0.12));
      f47+=tri(q,vec2(-0.18,0.0),vec2(0.54,0.84),vec2(0.38,0.12));
      f47+=tri(q,vec2(0.44,-0.10),vec2(0.92,-0.56),vec2(0.74,-0.04));
      f47+=tri(q,vec2(0.44,0.10),vec2(0.92,0.56),vec2(0.74,0.04));
      float f47lines=0.0;
      f47lines+=seg(q,vec2(-1.05,0.0),vec2(0.74,0.0),0.018);
      f47lines+=seg(q,vec2(-0.18,-0.08),vec2(0.54,-0.62),0.014);
      f47lines+=seg(q,vec2(-0.18,0.08),vec2(0.54,0.62),0.014);
      f47lines+=seg(q,vec2(-0.52,-0.05),vec2(-0.10,-0.05),0.014);
      f47lines+=seg(q,vec2(-0.52,0.05),vec2(-0.10,0.05),0.014);
      float canopy=ellipse(q,vec2(-0.62,0.0),vec2(0.20,0.085));
      color+=vec3(0.08,0.26,0.30)*f47*.38+vec3(0.42,1.0,0.72)*f47lines*.45+vec3(0.65,1.0,1.0)*canopy*.12;
      alpha=max(alpha,clamp((f47+f47lines*.65+canopy*.2)*.33,0.0,.58));

      float vignette=smoothstep(0.0,0.08,p.x)*(1.0-smoothstep(0.94,1.0,p.x))*smoothstep(0.0,0.10,p.y)*(1.0-smoothstep(0.94,1.0,p.y));
      color*=vignette;
      alpha*=vignette;
  ...
  const loc={
    ...
    cannon:gl.getUniformLocation(program,'uCannon')
  };
  gl.enable(gl.BLEND);
  gl.blendFunc(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA);
  return {
    ...
    draw(time,intensity,cannon=0){
      ...
      gl.uniform1f(loc.time,time);
      gl.uniform1f(loc.intensity,intensity);
      gl.uniform1f(loc.mode,mode==='rear'?1:0);
      gl.uniform1f(loc.cannon,cannon);
      gl.drawArrays(gl.TRIANGLES,0,6);
    }
  };
}
...
function resize(){
  ...
  shipSideRenderer?.resize(DPR);
  shipRearRenderer?.resize(DPR);
}
