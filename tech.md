...
### 10.1 待「站主真机验收」清单（沙盒无法渲染，代理无法代劳，这是唯一悬而未决的关闭条件）

- U46 ㉓：术语浮层触屏手感、阅读模式三档观感、横屏提示不烦人、各页人格未被稀释。
- U45 ⑲：年化面板上移后标题→面板视觉衔接（桌面+移动端地址栏可见态）、星门滚动叙事完整性、`?fx=stage` 不回归。
- U44 ⑰：stardrive 收缩过渡交界、pick-card hover/点按双态、鼠标视差幅度、分栏联动、窄屏回退、RM 全静态。
- U43 ⑫：games 区间刷子把手手感、中段平移、双指收放方向、滚轮回退、键盘可达、单段↔全程往返（⑨⑩⑪已过 vitest+build，仅缺触屏手感）。
- U42 ⑧：3D 数据星域全景/聚焦/移动端/筛选/弹窗/D-pad，通过后裁决转默认或保持 opt-in。
- U41：signal.html 编辑部重构——大标题气场、章节轨手感、指针摆动、筛选 chips、移动端单列排版。
- U40：games WC26 藏青+金配色观感、金字小字号可读性、决赛/季军赛倒计时与刮刮卡。
- U39：games 双指缩放三档方向、+/− 按钮、触控板 ctrl+滚轮。
- U38：games 滑杆换轮手感、thumb 过渡、点球比分、EN/中切换。
- U30：`?fx=stage` 星门舞台缩放幅度/圆角/视差强度，转默认或继续调。
- U24：起飞（甲板机位→尾追爬升→入列）/降落（塔台→掠过→甲板着舰）镜头链逐段过目，帧率关注。
- U25 revert 后：Combat View 应回到 U24 态（3D 默认+导演运镜，无 bloom/冲击波/真投影 HUD）。
- U23：Command 模式默认 3D 俯视战场+导演运镜、`?combatview=2d` 完整回退、boot.html 无按钮自动进入；`feida.au/boot.html` 默认+`?combatcam=tactical` 两态体验，决定 C 方案（全游戏化 boot）是否转正。
- U12b：首页顶栏 4 项无 Home、langBtn 位置、hover 跃迁特效。
- U16：serial 箭头消失+←/→不跳页、其余 8 页翻页不受影响、移动端「停止翻页」、书签按钮不溢出。
- U17：course.html Ch00 大表移动端横向滚动、外链可点、Ch06 测评题排版、周报占位样式。
- U18：stats.html 桌面+手机排版、图表可读性、`/league.html` 302、Labs 菜单「战绩」项。
- U19：stats tooltip 移动端 tap 行为、bootstrap 动画帧率、滑杆触达面积。
- U6：移动端整体友好度（iPhone 截图复核）。

### 10.2 待「站主裁决」的架构/范围决策（非验收，是要拍板）

- **U23 首页默认 3D 场景架构**（RFC `rfcs/2026-07-13-u23-default-3d-scene.md`）：① B→A 路线（Combat View 默认 3D 化→单 renderer 3D 舞台化，C 全游戏化永久否决）是否通过；② 移动端 M2 默认档位（RFC 建议 T1 起步）；③ starfield Worker 线是否保留为 T1 兜底。M1（Combat View 默认 3D 化）已代码完成待验收（见 10.1）。
...
### 10.3 roadmap.md 队列 B 未完成项（原样迁移，按原优先级）
...
### 10.4 roadmap.md 队列 A 未完成/触发式项

- **A2 main.js 拆分 Phase 4 剩余**（state 飞行状态机/nav/boot）+ **Phase 5**（styles.css `@layer` 分层）：均需真机验证 WebGL/canvas 视觉结果，不在沙盒强做。
- **B6 首页 WebGL 收尾**：`saturnRenderer` 等 raw-GL 渲染器补完整 context-restored 重建；跃迁点 shader 弱机自适应；统一渲染器 `powerPreference`/dpr 上限（现 1.75 vs 1.5 不一致）。需真机 profiling。
...
- **V18/V20/V21/V22/V23 各 Phase**：代码全部完成，**均待真人浏览器验收**（同 §10.1 性质，未逐条复制——详见已删除的 roadmap.md 历史版本 / git log，验收标准与 U 系列相同：沙盒无法渲染，站主真机确认才算关闭）。
